...
5.	Extra Mode: using the alternative powerup which means it boosts the speed up (x2) instead up growing
...
Controller takes care of taking information from the server and updating the world.
...
We also removes disconnected clients from the server, to speed up, especially when sending message is on the main thread. 
We didn't remove disconnected snakes from the world. Instead, after their disconnection death animation is complete, we ignore them. There won't be 
any calculation done for these snakes. The reason why we didn't remove them is that they would appear to be not moving on healthy clients' screen forever. 
Since we only handle a small number of players, we think sending disconnected snakes is not expensive, so they are not deleted.
